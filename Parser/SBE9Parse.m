...
%SBE9PARSE Parses a .cnv or .hex data file from a Seabird SBE9plus / 911 plus 
% CTD recorder.
%
% This function is able to read in a .cnv or .hex data file retrieved 
% from a Seabird SBE9plus / SBE911 CTD recorder. It makes use of two lower level
% functions, readSBE9hex and readSBE9cnv. The files consist of up to
% three sections: 
%
...
%
  ...
  if strcmpi(ext, '.hex')
    [data, comment] = readSBE9hex(dataLines, instHeader);
  else
    [data, comment] = readSBE9cnv(dataLines, instHeader, procHeader, mode);
  end
  ...
  if isfield(instHeader, 'instrument_model')
    ...
    sample_data.meta.instrument_model = 'SBE9 / 911';
  end
...
%PARSEINSTRUMENTHEADER Parses the header lines from a SBE9/911 .cnv file.
...
%% Work to be still done - get the necessary data parsed %% - Shawn M - nov 8, 2018

headerExpr   = '^\*\s*(SBE \S+|11Plus)\s+V\s+(\S+)';
headerExpr2  = 'Temperature SN =''(\S+)';
headerExpr3  = 'Conductivity SN =''(\S+)';
headerExpr4  = 'Sea-Bird (.*?) *?Data File\:';
...
exprs = {...
    headerExpr   headerExpr2    headerExpr3    headerExpr4    scanExpr     ...
